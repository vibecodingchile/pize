...
## Cumplimiento (Chile) – Ley 19.628

Este repo incluye una guía técnica y checklist de despliegue para alinearse con la Ley 19.628:
- `docs/compliance/ley_19628.md`
- `docs/compliance/security_checklist.md`

Incluye tests de no-regresión para patrones chilenos (RUT y teléfonos):
- `tests/test_chile_pii.py`


# Chile (RUT / Teléfonos / Patentes)
Este fork incluye detecciones por regex y anonimización sintética orientadas a Chile:
- `RUT_CL`, `PHONE_CL`, `LICENSE_PLATE_CL` en `data/regex_definition.csv`.
- Reemplazos con `Faker("es_CL")` en `anonymize.py` (clase `AllAnonym`).

Prueba rápida (local):
```bash
python -m venv venv
source venv/bin/activate
pip install -r min_req_demo.txt
python demo.py
```

# Cumplimiento Ley 19.628 (Chile)
Para operar en nivel pro:
- Lee `docs/compliance/ley_19628.md`.
- Usa el checklist `docs/compliance/security_checklist.md`.
- Completa la plantilla DPIA `docs/compliance/dpia_template.md` por caso de uso.

## Tests anti-fuga (recomendado)
Instala dependencias de desarrollo y ejecuta tests:
```bash
pip install -r min_req_demo.txt
